...
tar_read(p_bar_n2o_biochar1, store = here("_targets_biochar1"))
```

```{r, results = "hold", fig.show = "hold", echo = FALSE, fig.cap = "CO2 flux over time by treatment."}
tar_read(p_flux_co2_biochar1, store = here("_targets_biochar1"))
```

```{r, echo = FALSE, fig.cap = "CH4 flux over time by treatment."}
tar_read(p_flux_ch4_biochar1, store = here("_targets_biochar1"))
```

```{r, echo = FALSE, fig.cap = "N2O flux over time by treatment."}
tar_read(p_flux_n2o_biochar1, store = here("_targets_biochar1"))
```

```{r, echo = FALSE, fig.cap = "N2O flux versus temperature by treatment."}
tar_read(p_flux_n2o_T_biochar1, store = here("_targets_biochar1"))
```
...
tar_read(p_flux_n2o_with_Nappl_biochar1, store = here("_targets_biochar1"))
```

```{r, results = "hold", fig.show = "hold", echo = FALSE, fig.cap = "Diurnal variation in N2O flux."}
tar_read(p_flux_n2o_diurnal_biochar1, store = here("_targets_biochar1"))
```
