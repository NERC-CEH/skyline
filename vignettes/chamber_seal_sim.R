...
get_ts_chi <- function(
  ...
  chi_t <- (chi_t_flux + epsilon_anal) * seal +
           chi_ambient * (1 - seal)
  dt <- data.table(flux = flux, sigma_anal = sigma_anal, 
    t = v_t, chi_t_flux = chi_t_flux, chi_t = chi_t)
  return(dt)
}
...
get_dt_flux <- function(
  ...
  v_flux <- set_units(seq(from = 0, to = 5, length.out = n_sim), nmol_n2o/m^2/s)
  l_dt <- list()
  for (i in 1:n_sim) {
  ...
  dt[, r2 := summary(lm(chi_t ~ t, data = .SD))$r.squared, by = .(flux, sigma_anal)]
  dt[, rmse := summary(lm(chi_t ~ t, data = .SD))$sigma, by = .(flux, sigma_anal)]
...
npc_txt <- geom_text_npc(aes(npcx = 0.5, npcy = 0.9, label = round(nonlin*1e3)), size = 4)
...
p <- p + stat_smooth(method = "lm", colour = "red")
p <- p + stat_smooth(method = "gam", colour = "blue")
npc_txt <- geom_text_npc(aes(npcx = 0.5, npcy = 0.9, label = round(nonlin, 2)), size = 4)
# label <-  textGrob(label = round(nonlin, 2), x = .5, y = 0.9)
# p <- p + annotation_custom(label)
p <- p + facet_wrap(~ reorder(mmnt_id, -nonlin, mean))
p <- p + npc_txt
...
dt
plot_chi(l_out_biochar1$dt_chi[mmnt_id == "2021-05-19_01_02"], gas_name = "chi_co2")
