...
check_data_available <- function(
  ...
  if (nrow(dt) != 1) {
    stop(paste("Duplicate meta-data rows found in", fname_meta))
  }
  ...
  if (!is.na(v_ind[1]) && v_ind[1] > 1) {
    v_ind <- c(v_ind[1] - 1, v_ind)
  }
  ...
  return(list(
    v_fnames_ghg = v_fnames_ghg,
    v_fnames_pos = v_fnames_pos,
...
get_ghg_data <- function(
  ...
  # subset metadata to site and experiment
  dt_expt <- l_meta$dt_expt[this_site_id == site_id & this_expt_id == expt_id][
    1
  ]
  ...
  if (dt_expt$GHG_instrument == "Aeris MIRA Ultra") {
    ...
    setnames(
      dt_ghg,
      c("[H2O]_ppm", "chi_co2", "chi_ch4", "[N2O]_ppm", "GasP_torr", "GasT_C"),
      v_names,
      skip_absent = TRUE
    )
    ...
    setnames(
      dt_ghg,
      c("H2O", "CO2_dry", "CH4_dry", "N2O_dry", "CavityPressure", "CavityTemp"),
      v_names,
      skip_absent = TRUE
    )
    ## TODO: set instrument-specific units here too
...
get_ch_position_data <- function(
  ...
  if (fs::path_ext(v_fnames[1]) == "csv") {
    l_dt <- lapply(v_fnames, fread)
  }
  if (fs::path_ext(v_fnames[1]) == "dat") {
  ...
  if ("DateTime" %in% names(dt)) {
    dt[, datect := as.POSIXct(DateTime)]
  }
  if ("TIMESTAMP" %in% names(dt)) {
    dt[, datect := as.POSIXct(TIMESTAMP)]
  }
...
get_soilmet_data <- function(v_fnames, o2_data = TRUE) {
  if (fs::path_ext(v_fnames[1]) == "csv") {
    l_dt <- lapply(v_fnames, fread, na.strings = c("NAN", "NA", "NaN"))
  }
  if (fs::path_ext(v_fnames[1]) == "dat") {
    l_dt <- lapply(v_fnames, read_cs_data)
  }
  ...
  if ("RECORD" %in% names(dt)) {
    dt[, RECORD := NULL]
  }
  setnames(
    dt,
    c("C_Temp_C_Avg", "QR_Avg", "QR_C_Avg"),
  ...
  if ("PPFD_IN" %!in% names(dt)) {
    dt[, PPFD_IN := NA]
  }
  if ("PPFD_IN_ch" %!in% names(dt)) {
    dt[, PPFD_IN_ch := NA]
  }
  if ("VWC" %!in% names(dt)) {
    dt[, VWC := NA]
  }
  if ("TSoil" %!in% names(dt)) {
    dt[, TSoil := NA]
  }
  if ("SoilPerm" %!in% names(dt)) {
    dt[, SoilPerm := NA]
  }
...
get_data <- function(
  ...
  if (diagnostic_plots) {
    # default to FALSE but really helpful for detecting problems with system
    pname_diagnostic_plots <- here(
      "output",
      this_site_id,
      this_expt_id,
      "png",
  ...
  for (i in seq_along(v_dates)) {
    #seq_along equivalent to 1:length(v_dates), so processing one day at a time (/per loop)
    this_date <- v_dates[i]
    ...
    l_files <- check_data_available(
      ...
      this_expt_id,
      l_meta
    ) # check if data is available (function above)
    if (
      length(l_files$v_fnames_ghg) == 0 || length(l_files$v_fnames_pos) == 0
    ) {
      next
    ...
    dt_ghg <- get_ghg_data(
      l_files$v_fnames_ghg,
      this_date,
      this_site_id,
      this_expt_id,
      l_meta
    )
    dt_pos <- get_ch_position_data(
      l_files$v_fnames_pos,
      dt_expt$chpos_multiplier,
      this_date,
      this_site_id,
      this_expt_id,
      l_meta
    ...
    if (dt_band$ch_pos_fill) {
      dt <- setnafill(
        dt,
        type = "locf",
        cols = c("chamber_id", "C_Voltage", "C_mV_residual")
      )
    }
    ...
    dt <- dt_met[
      dt,
      on = .(chamber_id = chamber_id, datect = datect),
      roll = TRUE
    ] # rolling join of dt_met with dt based on chamber id column

    if (diagnostic_plots) {
      skyline_diagnostic_plot(dt, this_date, pname_diagnostic_plots)
    }
    ...
    dt[,
      mmnt_id := paste(
        this_expt_id,
        lubridate::date(dt$datect),
        formatC(
          as.numeric(as.character(chamber_id)),
          width = 2,
          format = "d",
          flag = "0"
        ),
    ...
    my.geocode <- data.frame(
      lon = dt_site$longitude,
      lat = dt_site$latitude,
      address = dt_site$site_name
    )
    ...
    if (length(unique(dt$chamber_id)) < 2) {
      next
    } #
    # remove deadband and plot (function below)
    dt <- identify_deadband(
      dt,
  ...
  if (write_all & !dryrun) {
    ...
    fname <- paste0(
      pname_csv,
      "/dt_chi_",
      lubridate::date(v_dates[1]),
      "_",
      lubridate::date(v_dates[n_days]),
      ".csv"
    )
...
filter_data <- function(site_id, expt_id, l_meta) {
  ...
  p <- ggplot(
    dt1,
    aes(t, get(gas_name), colour = as.factor(exclude), group = mmnt_id)
  )
...
plot_data_unfiltered <- function(
  ...
  p <- p +
    ...
    geom_vline(data = dt1, aes(xintercept = start_final_deadband))
...
plot_chi <- function(dt, gas_name = "n2o", rm_db = F) {
  if (rm_db == TRUE) {
    dt <- dt[w != 0, ]
  } # removes deadbands for plotting if chosen
  p <- ggplot(
    dt,
    aes(
      t,
      get(paste0("chi_", gas_name)),
      colour = as.factor(seq_id),
      group = mmnt_id
    )
  ) # colour as seq_id allows us to identify if fluxes were day or night and therefore identify any problems
  # p <- p + geom_point(alpha = 0.1) ## WIP setting alpha adds computation time - try without
  ...
  if (paste0("chi_pred_", gas_name) %in% colnames(dt)) {
    p <- p +
      geom_line(aes(y = get(paste0("chi_pred_", gas_name))), colour = "red")
  }
...
combine_fluxes <- function(site_id, expt_id) {
  ...
  fname <- here(
    ...
    paste0(
      "dt_flux_",
      lubridate::date(dt$datect[1]),
      "_",
      lubridate::date(dt$datect[length(dt$datect)]),
      ".csv"
    )
  )
  fwrite(dt, file = fname)
...
final_fluxes <- function(site_id, expt_id, l_meta) {
  ...
  for (i in 1:nrow(dt_badd)) {
    dt <- dt %>%
      dplyr::filter(
        !(chamber_id == dt_badd$chamber_id[i] &
          datect > dt_badd$start_time[i] &
          datect < dt_badd$end_time[i])
      )
  }
  fname <- here(
    "output",
...
plot_flux <- function(
  ...
  if (is.na(y_max)) {
    y_max <- max(dt_flux[, ci_hi])
  }
  if (is.na(y_min)) {
    y_min <- min(dt_flux[, ci_lo])
  }
...
plot_nema_vs_time <- function(
  ...
  # subset to just this experiment
  dt <- dt[
    site_id == this_site_id & expt_id == this_expt_id & chamber_id == "1"
  ]

  p <- ggplot(
    drop_units(dt[site_id == this_site_id & expt_id == this_expt_id]),
    aes(datect, f_n2o)
  )
  p <- p + geom_vline(data = dt_mgmt, aes(xintercept = start))
  p <- p + geom_hline(yintercept = 0)
...
plot_n2o_flux_diurnal <- function(
  dt_flux,
  flux_name = "f_N2O_dry",
  sigma_name = "sigma_N2O_dry",
  this_site_id,
  this_expt_id,
  mult = 1000,
  y_min = -2,
  ...
  p <- p +
    geom_errorbar(aes(ymin = ci_lo, ymax = ci_hi), colour = "dark orange")
  p <- p + geom_hline(yintercept = 0)
...
plot_flux_vs_xvar <- function(
  ...
  if (rows_only) {
    p <- p +
      facet_wrap(
        ~ get(facet_name),
        nrow = length(unique(dt[, get(facet_name)]))
      )
  } else {
    p <- p + facet_wrap(~ get(facet_name))
  }
...
finding_Nema <- function(dt_flux, l_meta, save_file = FALSE) {
  ...
  first_date_flux <- lubridate::floor_date(
    min(as.POSIXct(dt_flux$datect)),
    "day"
  )
  last_date_flux <- lubridate::floor_date(
    max(as.POSIXct(dt_flux$datect)),
    "day"
  )
  ...
  dt_mgmt <- dt_mgmt[, .(
    ...
    trmt_id,
    event_id,
    datect = as.POSIXct(round(start, "days")),
    N_appl_amount_kg.ha
  )]
...
filter_fluxes <- function(dt, save_file = FALSE, fname = "dt_flux") {
  ...
  if (save_file) {
    fwrite(dt, file = here("output", paste0(fname, ".csv")))
  }
  if (save_file) {
    qsave(dt, file = here("output", paste0(fname, ".qs")))
  }
  return(dt)
}

filter_fluxes2 <- function(
  dt,
  save_file = FALSE,
  fname = "dt_flux",
  rmse_threshold = 3
  ...
  if (save_file) {
    fwrite(
      dt,
      file = here("output", dt[1, .(site_id, expt_id)], paste0(fname, ".csv"))
    )
  }
...
plot_means_by_trmt <- function(
  ...
  if (by_chamber) {
    dt_summary <- dt[,
      .(f = mean(f, na.rm = TRUE), f_sd = sd(f, na.rm = TRUE), n = .N),
  ...
  if (by_chamber) {
    p <- p +
      geom_pointrange(
        aes(ymin = ci_lo, ymax = ci_hi, colour = trmt_id),
...
plot_diurnal <- function(
  ...
  if (split_by_day & split_by_expt & split_by_chamber) {
    dt_flux[,
      f_co2_scaled := scale(f_co2),
      by = .(date_day, expt_id, chamber_id)
    ]
    dt_flux[,
      f_n2o_scaled := scale(f_n2o),
      by = .(date_day, expt_id, chamber_id)
    ]
    dt_flux[, TA_scaled := scale(TA), by = .(date_day, expt_id, chamber_id)]
    dt_flux[,
      TSoil_scaled := scale(TSoil),
      by = .(date_day, expt_id, chamber_id)
    ]
    dt_flux[,
      PPFD_IN_scaled := scale(PPFD_IN),
      by = .(date_day, expt_id, chamber_id)
    ]
  ...
  p <- p +
    stat_smooth(aes(y = f_co2_scaled * -1, colour = "CO2 flux"), method = "gam")
  p <- p +
    stat_smooth(aes(y = f_n2o_scaled, colour = "N2O flux"), method = "gam")
  if (split_by_expt) {
...
get_gamdiff <- function(dt) {
  ...
  dt[
    exclude == FALSE,
    sigma_lm := summary(lm(chi_co2 ~ t, w = w, data = .SD))$sigma^2,
    by = mmnt_id
  ]
  dt[
    exclude == FALSE,
    sigma_gam := summary(gam(
      chi_co2 ~ s(t),
      weights = w / mean(w),
      data = .SD
    ))$scale,
...
plot_chi_co2_with_rmse <- function(
  dt,
  n = 0,
  add_gam = FALSE,
  save_plot = FALSE
  ...
  p <- ggplot(
    dt[exclude == FALSE],
    aes(t, chi_co2, colour = rmse_f_co2, group = mmnt_id, weight = w)
  )
  p <- p + geom_point() ## WIP setting alpha adds computation time - try without
  p <- p + geom_line(aes(y = chi_pred_co2), colour = "red", linewidth = 1)
  if (add_gam) {
    p <- p + geom_line(aes(y = chi_pred_gam), colour = "orange", linewidth = 1)
  }
  npc_txt <- geom_text_npc(
    aes(npcx = 0.5, npcy = 0.9, label = round(rmse_f_co2, 2)),
    size = 4
  )
  p <- p + npc_txt
...
calc_flux_nl <- function(
  ...
  if (any(dt$linear == FALSE)) {
    ...
    dt[
      linear == FALSE,
      sigma_dchi_dt := summary(lm(form, w = w, data = .SD))$coefficients[2, 2],
      by = mmnt_id
    ]
    dt[linear == FALSE, (flux_var_name) := dchi_dt * rho * volume_m3 / area_m2]
    dt[
      linear == FALSE,
      (sigma_var_name) := sigma_dchi_dt * rho * volume_m3 / area_m2
    ]
    dt[
      linear == FALSE,
      (r2_var_name) := summary(lm(form, w = w, data = .SD))$r.squared,
      by = mmnt_id
    ]
    dt[
      linear == FALSE,
      (rmse_var_name) := summary(lm(form, w = w, data = .SD))$sigma,
      by = mmnt_id
    ]
    dt[
      linear == FALSE,
      (pred_var_name) := predict(lm(form, w = w, data = .SD)),
      by = mmnt_id
    ]
  }
...
partition_fluxes <- function(
  dt,
  method = c("subtract_nighttime_R", "regression")
) {
...
switch_sign_co2 <- function(
  ...
  if (convention != convention_in) {
    stop(
      "Attempting to switch sign convention
...
expand_to_complete_ts <- function(
  ...
  dt <- dt[, ..cols][
    dt_time,
    on = .(datect = date_byhour, chamber_id = chamber_id)
  ]
...
fill_gaps_PPFD_dTA_VWC <- function(dt) {
  ...
  if (any(a_counts < 10)) {
    # there are weeks with less than 10 hours with data
    # put the week term inside the smoother so not all independent
    dt[,
      PPFD_pred := predict(
        mgcv::gam(PPFD_IN ~ s(hour, bs = "cc", by = week), data = .SD),
        newdata = .SD
      )
    ]
  } else {
    ...
    dt[,
      PPFD_pred := predict(
        mgcv::gam(PPFD_IN ~ s(hour, bs = "cc"), data = .SD),
        newdata = .SD
      ),
      by = week
    ]
  ...
  if (any(a_counts < 10)) {
    ...
    dt[,
      dTA_pred := predict(
        mgcv::gam(dTA ~ PPFD_IN + s(hour, bs = "cc", by = week), data = .SD),
        newdata = .SD
      )
    ]
  } else {
    ...
    dt[,
      dTA_pred := predict(
        mgcv::gam(dTA ~ PPFD_IN + s(hour, bs = "cc"), data = .SD),
        newdata = .SD
      ),
      by = week
    ]
  ...
  dt[,
    VWC_pred := predict(mgcv::gam(VWC ~ s(datets), data = .SD), newdata = .SD)
  ]
  # replace missing values with predictions
...
get_cum_f_co2 <- function(dt) {
  ...
  if (interval_length != 3600) {
    stop(
      "I haven't checked this works time_interval
...
plot_vwc_response <- function(dt, alpha1 = 4, alpha2 = 6, save_plot = FALSE) {
...
plot_T_response <- function(
  ...
  p <- p + xlab(x_varname)
  if (logy) {
    p <- p + scale_y_continuous(trans = scales::log_trans())
  }
...
plot_hist_fn2o <- function(dt, logy = FALSE, save_plot = FALSE) {
  ...
  if (logy) {
    p <- ggplot(drop_units(dt), aes(x = log(f_n2o)))
  }
  p <- p + geom_histogram()
  p <- p + facet_wrap(~expt_id)
  p
...
add_vwc_response <- function(dt, alpha1 = 4, alpha2 = 6, save_plot = FALSE) {
  dt[, f_n2o_rel := f_n2o / max(f_n2o), by = expt_id]
  dt[, f_vwc := dbeta(VWC, shape1 = alpha1, shape2 = alpha2)]
  dt[, f_vwc := f_vwc / max(f_vwc, na.rm = TRUE)]
...
skyline_diagnostic_plot <- function(dt, this_date, pname_diagnostic_plots) {
  # useful for EA but default to FALSE unless specified
  dt_subset <- dt %>% filter(row_number() %% 15 == 1)

  a <- ggplot(dt_subset, aes(datect, chamber_id)) +
    geom_point() +
    ggtitle("Chamber Position") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  b <- ggplot(dt_subset, aes(datect, PPFD_IN_ch)) +
    geom_point() +
    ggtitle("Chamber PAR") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  c <- ggplot(dt_subset, aes(datect, TA)) +
    geom_point() +
    ggtitle("Chamber Temperature") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  d <- ggplot(dt_subset, aes(datect, chi_co2)) +
    geom_point() +
    ggtitle("CO2") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  e <- ggplot(dt_subset, aes(datect, chi_n2o)) +
    geom_point() +
    ggtitle("N2O") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  f <- ggplot(dt_subset, aes(datect, chi_h2o)) +
    geom_point() +
    ggtitle("H2O") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  g <- ggplot(dt_subset, aes(datect, P_cavity)) +
    geom_point() +
    ggtitle("CavityPressure") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
  h <- ggplot(dt_subset, aes(datect, T_cavity)) +
    geom_point() +
    ggtitle("CavityTemp") +
    scale_x_datetime(date_breaks = "6 hours", date_labels = "%H:%M")
