...
get_biological_flux <- function(
  ...
  Q_asymptotic = FALSE,
  T_exponential = FALSE,
  convention = c("biological", "micrometerological")
) {
...
get_photosynthesis <- function(lue, R_10, PPFD_IN, Q_asymptotic = FALSE) {
  if (Q_asymptotic == TRUE) {
    P <- (P_max * lue * PPFD_IN) /
      (P_max + lue * PPFD_IN)
  } else {
...
dt[,
  f_co2_pred := get_biological_flux(
    v_theta_ini[1],
    v_theta_ini[2],
    v_theta_ini[3],
    v_theta_ini[4],
...
fit <- optim(
  ...
  method = "L-BFGS-B",
  lower = 0,
  control = list(factr = 1e-15, trace = 3)
)
...
lapply(dt[, unique(chamber_id)], function(.chamber_id) {
  get_biological_flux(dt[chamber_id == .chamber_id, c(nm1, nm2), with = FALSE])
})
...
fit <- nls(
  f_co2 ~ lue * sqrt(PPFD_IN) - (R_10 + k_T * dTA),
  data = dt,
...
expand_to_complete_ts <- function(
  ...
  dt_pred <- dt[, ..cols][
    dt_time,
    on = .(date_byhour = date_byhour, chamber_id = chamber_id)
  ]
  ...
  sum(is.na(dt_pred$PPFD_IN))
  sum(is.na(dt_pred$dTA))
  sum(is.na(dt_pred$VWC))
  sum(is.na(dt_pred$f_co2))
  sum(is.na(dt_pred$lue))
  sum(is.na(dt_pred$R_10))
  sum(is.na(dt_pred$k_T))
  return(dt)
}
...
p <- ggplot(dtts[chamber_id == 1], aes(datect, VWC, colour = chamber_id))
p <- p + geom_point()
...
p <- ggplot(dt_pred, aes(datect, f_co2_obs_cum, colour = chamber_id))
p <- p + geom_line()
