...
for (i in seq_along(v_date)) {
    ...
    if (is.na(dtt$CHAMBER[1])) dtt$CHAMBER[1] <- 99
    if (is.na(dtt$TEMPK[1])) dtt$TEMPK[1] <- 99
    # carry chamber data over to subsequent seconds
    dtt[, CHAMBER := na.locf(CHAMBER)]
    dtt[, TEMPK := na.locf(TEMPK)]
#   summary(dtt)
    dt_out <- dtt[CHAMBER != 99, .(TIMESTAMP = datect_lgr, C_Voltage = CHAMBER, TA = TEMPK - 273.15)]
