...
# Set target options:
v_pkgs = c("here", "fs", "data.table", "readxl", "units", "ggplot2",
...
#### list of targets: ####
  ...
  list(
    ...
    tar_target(
      name = dt_flux,
      command = filter_fluxes2(dt, save_file = TRUE, fname = "dt_flux", rmse_threshold = 6.5)
    ),
    ...
    tar_target(
      name = dt_final_filter,
      command = final_fluxes(dt = dt_flux, this_site_id = site_id, this_expt_id = expt_id, l_meta = l_meta)
    ),

    tar_target(
      name = dt_final,
      command = fwrite(dt_final_filter, file = paste0(here("output", dt_final_filter[1, .(site_id, expt_id)]), "/dt_flux.csv"))
